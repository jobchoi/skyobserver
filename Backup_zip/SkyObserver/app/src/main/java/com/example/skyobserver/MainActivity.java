...
import static com.example.skyobserver.member.Login.sDto;
...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       /* DrawerLayout drawer = findViewById(R.id.drawer_layout);
        ...
        imageView = headerview.findViewById(R.id.headerimageView);
...
//        });
    ...
    protected void restoreState(){
        ...
        Log.d("userPref-RESTORE",userPref.getAll().values().toString());
        if (userPref.getAll().values().toString().length()>2 ) {
            // 필요한 형식을 가져오가너 getAll로 모든 값을 사용.
            Log.d("restoreState값확인 : ",userPref.getAll().values().toString());
            // 로그인 후 이름, 프로필 이미지 변경
            idtextView.setText(userPref.getString("email", ""));
            String profileimage = userPref.getString("filename","");
            Log.d("tt111111",profileimage);
    ...
    public void signCaller(View view) {
