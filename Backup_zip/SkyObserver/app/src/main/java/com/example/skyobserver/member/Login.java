...
public class Login extends AppCompatActivity {
        ...
        try {
            ...
            if (resEntity != null) {
                ...
                if (resPhoneStr.equals("NG")) {
                    ...
                    try {
                        navigationView = findViewById(R.id.headerimageView);
    ...
    protected void saveState(SiginDTO sDto) {
        ...
        if(sDto.getFilename().equals("null")){
            // 프로필 이미지가 없다면, 대체 이미지를 설정하는 부분을 구현
            Log.i("프로필이미지",sDto.getFilename());
            //sDto.setFilename("");
        } else{
            Glide.with(this).load(sDto.getFilename());
        }
